...
function drawCH4(m){
  // C central rendu plus visible (plus gros + plus sombre + contour renforcé)
  const cx = m.x, cy = m.y;
  const off = R*1.20;

  // Carbone central : rayon augmenté + contour légèrement plus marqué
  drawSphere(cx, cy, "#2b2b2b", "C", "rgba(0,0,0,0.35)", R*1.25);

  // Hydrogènes : légèrement plus petits, pour accentuer le C
  const rH = R*0.85;
  drawSphere(cx + off, cy, "#ffffff", "H", "rgba(0,0,0,0.35)", rH);
  drawSphere(cx - off, cy, "#ffffff", "H", "rgba(0,0,0,0.35)", rH);
  drawSphere(cx, cy + off, "#ffffff", "H", "rgba(0,0,0,0.35)", rH);
  drawSphere(cx, cy - off, "#ffffff", "H", "rgba(0,0,0,0.35)", rH);
